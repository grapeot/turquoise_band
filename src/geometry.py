"""【共享基础】视线几何：常数 + 切向（limb）穿透路径 + 擦边高度 ↔ 月面径向位置的解析映射。

注意：本模块的解析折射 α(h)/聚焦公式只供解析对照链（pipeline/render_rt/brute_ray_trace）
使用；权威管线的折射/聚焦走 refraction_trace（RK4 数值积分）与撒线涌现，不用这里的解析处方。
几何常数（R_EARTH/D_MOON_KM/umbra_radius 等）则为全项目共享。
