"""【解析对照】暴力 2D ray tracing —— 圆盘解析对照实现（raytrace_eclipse 分层抽样管线之前的对照，曾称"金标准"，现已被权威管线取代）。

完全独立、第一性原理。不查 a_signed 反插值、不用任何 focusing 公式。
注意：折射仍用解析 α(h)、消光仍是直线视线（radiative_transfer），无撞地遮挡/弯曲消光，
本影偏亮 −7.7 档；现仅作 ablation step5 与独立交叉验证用。
